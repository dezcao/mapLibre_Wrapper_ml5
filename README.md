...
## 이 래퍼를 만든 이유 (level1의 의도)

- **level1 (이 저장소)** — MapLibre를 얇게 감싼 **저수준 코어**. 지도 초기화와 점·선·면(아이콘·선·팝업)의 추가/갱신/제거 같은 **기본 동작만** 담당합니다. 특정 서비스·도메인 로직을 넣지 않아 **오염되지 않게** 유지하는 것이 목표입니다.
- **level2 (별도 구현)** — Vue/React 또는 사용자별 니즈에 맞춘 구현. level1을 로드한 뒤 그 위에서 동작케합니다. 비즈니스 로직, prop·상태 관리, 메시지 통신은 모두 이 층의 몫입니다.

경계는 단순합니다. level1은 **커스텀 이벤트**(`icon-click` 등)와 **단순한 함수 API**만 노출하고, 그 바깥(닫기 버튼 UI, 데이터 소스, 클릭 후 동작 등)은 level2에 위임합니다.
...
- 전역 함수명(`createMap` 등)은 버전 중립적이므로, 서로 다른 버전 세트를 **동시에 로드하면 충돌**합니다. 교차 버전 혼용(예: `ml6` + `ml5icon`)은 MapLibre의 하위 호환에 기대 동작할 수도 있으나, **이 코어가 보장하는 범위는 아닙니다**.
